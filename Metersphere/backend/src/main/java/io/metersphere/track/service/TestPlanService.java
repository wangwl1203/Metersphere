...
import io.metersphere.track.issue.domain.Jira.JiraIssueVersions;
...
public class TestPlanService {
    ...
    public String getVersion(String testPlanId,String projectId, String workSpaceId){
        return testPlanMapper.selectVersionById(testPlanId,projectId,workSpaceId);
    }

    public void updateVersionByPlanId(String projectId){
        testPlanMapper.updateVersionByPlanId(projectId);
    }

    public void updateIssuesInfoByPlanId(String planId){
    ...
    public List<JiraIssueVersions> getVersionList(String projectId){
        ...
        return jiraPlatform.getIssueVersion(project);
    }
    ...
    public TestPlan editTestPlan(TestPlanWithBLOBs testPlan) {
        ...
        // 测试计划，修改影响版本，相关处理
        if(StringUtils.isNotBlank(testPlan.getVersionIdOld()) && !testPlan.getVersionIdOld().equals(testPlan.getVersionId())){
            String planId = testPlan.getId();
            ...
            // 如果存在测试计划、用例关联记录则继续处理，如果无记录结束
            if(testPlanTestCaseWithBLOBs.size()>0){
                // 查询出选择的最新影响版本下的所有缺陷（查第三方Jira）
                IssuesRequest request = new IssuesRequest();
                request.setProjectId(testPlan.getProjectId());
                request.setPlanId(testPlan.getId());
                // 记录所有三方缺陷id
                List<String> issuesFromJira = new ArrayList<>();
                // 查询三方缺陷总条数
                Integer issuesTotal = issuesService.issueCount(request);
                if(issuesTotal > 0){
                    // 根据分页，查询三方缺陷所有数据
                    List<IssuesDao> issuesDaoList = issuesService.syncJiraIssues(request,1,issuesTotal,"");
                    if(issuesDaoList.size() >0 ){
                        issuesDaoList.forEach(item->{
                            issuesFromJira.add(item.getId());
                        });
                    }
                }
                ...
                // 遍历test_plan_test_case记录
                for(TestPlanTestCaseWithBLOBs item: testPlanTestCaseWithBLOBs){
                    // 如果issues为空、issues_count=0，则结束
                    String issuesJson = item.getIssues();
                    Integer issuesCount = item.getIssuesCount();
                    if(issuesJson != null && issuesCount != 0){
                        ...
                        for(int num = issuesList.size()-1; num >= 0; num--){
                            ...
                            // 判断issuesId是否在， 最新影响版本的缺陷列表中, 如果存在直接结束, 不存在继续处理
                            if(!issuesFromJira.contains(issuesId) && IssuesManagePlatform.Jira.toString().equals(platform)){
                                // 不在 最新影响版本缺陷列表中，需要将test_plan_test_case表的issues字段去除当前的json,issues_count-1
                        ...
                        if(copyIssuesList.size()==0 && issuesCount ==0){
                            ...
                            item.setIssues(copyIssuesList.toString());
                            item.setIssuesCount(issuesCount);
                            testPlanTestCaseMapper.updateIssuesByPlanId(item);
                        }
                    }
                }
    ...
    public void editTestPlanStatus(String planId) {
        ...
        SaveTestPlanRequest testPlanRequest = new SaveTestPlanRequest();
        TestPlan testPlanExt = testPlanMapper.selectByPrimaryKey(planId);
        if (StringUtils.equals(TestPlanStatus.Completed.name(), testPlanExt.getStatus())) {
            try {
                org.apache.commons.beanutils.BeanUtils.copyProperties(testPlanRequest, testPlanExt);
                String context = getTestPlanContext(testPlanRequest, NoticeConstants.Event.UPDATE);
                Map<String, Object> paramMap = new HashMap<>(getTestPlanParamMap(testPlanRequest));
                ...
                NoticeModel noticeModel = NoticeModel.builder()
                        ...
                        .status(TestPlanStatus.Completed.name())
                        .build();
...
//            request.getConfig().getReportType()
    ...
    public TestPlan copy(String planId) {
        ...
        targetPlan.setVersionId(testPlan.getVersionId());
        targetPlan.setRepeatCase(testPlan.getRepeatCase());
...
//                        performanceReportService.getReportLogs(reportId, resourceId);
    ...
    public TestPlanSimpleReportDTO getReport(String planId) {
        ...
        issuesService.calculatePlanReport(planId, report,testPlan.getProjectId());
