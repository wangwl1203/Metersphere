...
import io.metersphere.excel.listener.WorkSpaceUserDataListener;
...
public class UserService {
    ...
    public void userTemplateExport(HttpServletResponse response, String type) {
        String template_name = "user_import_template_name";
        String template_sheet = "user_import_template_sheet";
        try {
            if(type.equals("workSpace")){
                template_name = "workSpace_user_import_template_name";
                template_sheet = "workSpace_user_import_template_sheet";
                EasyExcelExporter easyExcelExporter = new EasyExcelExporter(new WorkSpaceUserExcelDataFactory().getExcelDataByLocal());
                easyExcelExporter.export(response, generateWorkSpaceExportTemplate(),
                        Translator.get(template_name), Translator.get(template_sheet));
    ...
    private List<WorkSpaceUserExcelData> generateWorkSpaceExportTemplate() {
        List<WorkSpaceUserExcelData> list = new ArrayList<>();
        for (int i = 1; i <= 2; i++) {
            WorkSpaceUserExcelData data = new WorkSpaceUserExcelData();
        ...
        list.add(new WorkSpaceUserExcelData());
        WorkSpaceUserExcelData explain = new WorkSpaceUserExcelData();
        ...
        explain.setProMemberProject("多个项目以换行分隔");
        list.add(explain);
        return list;
    }
    ...
    public ExcelResponse workSpaceUserImport(MultipartFile multipartFile, HttpServletRequest request, String workSpaceId) {
        ...
        try {
            Class clazz = new WorkSpaceUserExcelDataFactory().getExcelDataByLocal();
            ...
            EasyExcelListener easyExcelListener = new WorkSpaceUserDataListener(clazz, workspaceNameMap, projectNameMap, workSpaceId, workSpaceName);
            EasyExcelFactory.read(multipartFile.getInputStream(), clazz, easyExcelListener).sheet().doRead();
            if (CollectionUtils.isNotEmpty(((WorkSpaceUserDataListener) easyExcelListener).getNames())) {
                request.setAttribute("ms-req-title", String.join(",", ((WorkSpaceUserDataListener) easyExcelListener).getNames()));
                request.setAttribute("ms-req-source-id", JSON.toJSONString(((WorkSpaceUserDataListener) easyExcelListener).getIds()));
            }
            errList = easyExcelListener.getErrList();
        } catch (Exception e) {
            LogUtil.error(e.getMessage(), e);
