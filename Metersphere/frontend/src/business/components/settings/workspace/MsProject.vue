        ...
        <el-form-item :label-width="labelWidth" :label="$t('project.zentao_id')" v-if="zentao">
          ...
          <ms-instructions-icon effect="light">
            <template>
              ...
              项目-迭代 | 项目-冲刺 需要填写项目ID<br/><br/>
              产品ID取值:<br/><br/>
              <img class="jira-image" src="../../../../assets/zentao-key.png"/>
            </template>
          </ms-instructions-icon>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.jira_key = this.form.jiraKey;
    ...
    submit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if(saveType === "update"){
            if(this.jira_key !== this.form.jiraKey && this.jira_key !== null){
              this.$confirm("[JIRA项目key]修改:  [测试计划]、[测试用例], 已关联的[影响版本]、[缺陷]都将被清除！",'', {
                confirmButtonText: this.$t('commons.confirm'),
                cancelButtonText: this.$t('commons.cancel'),
                type: 'warning'
              }).then(() => {
                this.$confirm("请再次确认, 清除无影响！", '',{
                  confirmButtonText: this.$t('commons.confirm'),
                  cancelButtonText: this.$t('commons.cancel'),
                  type: 'warning'
                }).then(() => {
                  ...
                  this.result = this.$post("/project/updatePlanIssues", condition, () => {
                    this.operator(saveType);
                  });
                }).catch(() => {
                });
              }).catch(() => {
