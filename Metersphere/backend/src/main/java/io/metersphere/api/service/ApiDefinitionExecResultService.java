        ...
        if (startTime == null) {
            ...
            for (int i = 0; i < list.size(); i++) {
                if (i < limitNumber) {
                    ...
                    } else if ("functional".equals(item.getCaseType())) {
                        planRequest.setFunctionId(item.getTestCaseID());
                    }
                    List<TestPlanDTO> dtoList = testPlanService.selectTestPlanByRelevancy(planRequest);
                    item.setTestPlanDTOList(dtoList);
                    if("functional".equals(item.getCaseType())){
                        item.setFailureTimes(new Integer(dtoList.size()).longValue());
                    }
                    returnList.add(item);
                } else {
                    break;
            ...
            Comparator<ExecutedCaseInfoResult> bya = Comparator.comparing(ExecutedCaseInfoResult::getFailureTimes).reversed();//按照FailureTimes降序
            Comparator<ExecutedCaseInfoResult> byb = Comparator.comparing(ExecutedCaseInfoResult::getUpdateTime).reversed();//按照UpdateTime降序
            returnList.sort(bya.thenComparing(byb));//按照FailureTimes降序再按照UpdateTime降序
