...
//        }
    ...
    private TestCaseWithBLOBs convert2TestCase(TestCaseExcelData data) {
        ...
        testCase.setStatus(data.getStatus());
    ...
    public String dataOperator(String row){
        if(!row.contains(".")){
            Pattern pattern = Pattern.compile("^[0-9]+");
            Matcher matcher = pattern.matcher(row);
            matcher.find();

            String[] detail = row.split("^[0-9]+");
    ...
    public String dataOperatorExt(String row){
        Pattern pattern = Pattern.compile("^[0-9]+");
        Matcher matcher = pattern.matcher(row);
        if(matcher.find()){
            // 包含序号，继续处理
            ...
            // 不包含序号，则抛错
            throw new RuntimeException(Translator.get("step_or_stepResult_error"));
        }
    }
    public String getSteps(TestCaseExcelData data) {
        ...
        if (data.getStepDesc() != null) {
            ...
            for (String row : stepDesc) {
                //row = row.replaceAll("(^[sS])","");
                //row = this.dataOperator(row);
                row = this.dataOperatorExt(row);
        ...
        if (data.getStepResult() != null) {
            ...
            for (String row : stepRes) {
                ...
                if (rowIndex > -1) {
                    ...
                    if(stepResList.size() >= rowIndex){
                        listUtils.set(stepResList, lastStepIndex - 1 , stepResList.get(lastStepIndex - 1) + "\n"+ stepBuffer.toString(), "");
                    }else{
                        listUtils.set(stepResList, lastStepIndex , stepBuffer.toString(), "");
                    }
