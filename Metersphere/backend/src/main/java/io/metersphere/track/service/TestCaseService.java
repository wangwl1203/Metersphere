            ...
            testCases.forEach((testCase) -> {
                ...
                for (int i = 0; i < fields.size(); i++) {
                    ...
                    if (StringUtils.equals(request.getCustomField().getName(), field.getName())) {
                        ...
                        // 当修改用例状态时，除修改test_case表custom_fields字段中的用例状态外，status也做修改
                        if(request.getCustomField().getName().equals("用例状态")){
                ...
                if (StringUtils.equals(request.getCustomField().getName(), "责任人")) {
...
//                        Collections.reverse(continueCaseList);
    ...
    public List<TestCaseTest> getRelateAutoTest(String testId) {
        ...
        List<TestCaseTest> testCaseTestList = new ArrayList<>(); // 存放最终处理的数据
        testCaseTests.forEach(item -> {
            // 获取功能用例名称、功能用例描述
            String testCaseId = item.getTestCaseId();   // 功能用例id
            TestCaseWithBLOBs testCaseWithBLOBs = testCaseMapper.selectByPrimaryKey(testCaseId);

            // 获取项目id、项目名称
            String projectId = testCaseWithBLOBs.getProjectId(); // 项目id
            Project project = projectService.getProjectById(projectId);
