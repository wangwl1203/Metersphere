<template>
  <el-tabs class="other-info-tabs" v-loading="result.loading" v-model="tabActiveName">
    ...
    <el-tab-pane :label="$t('test_track.related_requirements')" name="demand" v-if="hasZentao">
<!--      <el-col :span="7">-->
...
<!--        </el-form-item>-->
<!--      </el-col>-->
      <test-case-demand-relate
        :plan-id="planId"
        :read-only="readOnly && !(isTestPlan)"
        :case-id="caseId" :is-issues-flag=isIssuesFlag ref="demand"/>
    </el-tab-pane>

    <el-tab-pane :label="$t('test_track.case.relate_issue')" name="bug" v-if="isIssuesFlag">
...
import TestCaseDemandRelate from "@/business/components/track/case/components/TestCaseDemandRelate";
import {getCurrentProjectID} from "@/common/js/utils";

export default {
  name: "TestCaseEditOtherInfo",
  components: {
    TestCaseDemandRelate,
    TabPaneCount,
    DependenciesList,
    TestCaseTestRelate,
    FormRichTextItem, TestCaseIssueRelate, TestCaseAttachment, MsRichText, TestCaseRichText},
  props: ['form', 'labelWidth', 'caseId', 'readOnly', 'projectId', 'isTestPlan', 'planId', 'isIssuesFlag'],
  data() {
    return {
      result: {},
  ...
  methods: {
    getProject() {
      ...
      this.result = this.$get("/project/get/" + projectId, response => {
        let project = response.data;
        if(project.jiraKey != null && project.jiraKey !== ""){
          this.hasJira = true;
        }
        if(project.zentaoId != null && project.zentaoId !== ""){
          this.hasZentao = true;
        }
      });
    },
