...
public class XmindCaseParser {
    ...
    private static final String TC_REGEX = "(?:^[p][0-4]?\\.*\\s*)";
    private static final String PC_REGEX = "(?:^[c][1-9]*\\.*\\s*)";
    private static final String RC_REGEX = "(?:rc:|rc：)";
    private static final String ID_REGEX = "(?:id:|id：)";
    private static final String TAG_REGEX = "(?:tag: |tag： |tag:|tag：)";
    private static final String S_REGEX = "(?:^[s][1-9]*\\.*\\s*)";
    private static final String E_REGEX = "(?:^[e][1-9]*\\.*\\s*)";
    private static final String P_REGEX = "[p][0-4]?";
    ...
    private final List<String> priorityList = Arrays.asList("P0", "P1", "P2", "P3","P4");
...
//        }
    ...
    private String getSteps(List<Attached> attacheds) {
        ...
        if (!attacheds.isEmpty()) {
            for (int i = 0; i < attacheds.size(); i++) {
                ...
                step.put("num", i + 1);
                if (isAvailable(attacheds.get(i).getTitle(), S_REGEX)) {
                    step.put("desc", replace(attacheds.get(i).getTitle(), S_REGEX));
                }
                if (attacheds.get(i) != null && attacheds.get(i).getChildren() != null && attacheds.get(i).getChildren().getAttached() != null) {
                    String resultExt = "";
                    for(int j = 0; j < attacheds.get(i).getChildren().getAttached().size(); j++){
                        if (isAvailable(attacheds.get(i).getChildren().getAttached().get(j).getTitle(), E_REGEX)) {
                            resultExt += replace(attacheds.get(i).getChildren().getAttached().get(j).getTitle(), E_REGEX) + "\n";
                        }
                    }
                    step.put("result", resultExt.trim());
                    /*if (isAvailable(attacheds.get(i).getChildren().getAttached().get(0).getTitle(), E_REGEX)) {
                        step.put("result", replace(attacheds.get(i).getChildren().getAttached().get(0).getTitle(), E_REGEX));
                    }*/
                }
                jsonArray.add(step);
            }
    ...
    private void formatTestCase(String title, String nodePath, List<Attached> attacheds) {
        ...
        Pattern pattern = Pattern.compile(P_REGEX,Pattern.CASE_INSENSITIVE);// 匹配的模式    
        Matcher m = pattern.matcher(temp_title);
        if (m.find()) {
            priority= m.group(0).trim();
        }
//        String priority = replace(temp_title,name).trim().replace(".","");
        if (priority.toUpperCase().startsWith("P")) {
            testCase.setPriority(priority.toUpperCase());
        } else {
            testCase.setType(caseTypeMap.get(priority));
        }
        ...
        if (attacheds != null && !attacheds.isEmpty()) {
            attacheds.forEach(item -> {
                if (isAvailable(item.getTitle(), PC_REGEX)) {
                    ...
                    if(prerequisite != null && !prerequisite.equals("")){
                        // 前置条件，多条情况
                        testCase.setPrerequisite(prerequisite + "\n" + replace(item.getTitle(), PC_REGEX));
                    }else{
