...
import {GROUP_PROJECT, GROUP_WORKSPACE, TokenKey} from '../../../../../common/js/constants';
...
export default {
  ...
  methods: {
    ...
    downloadTemplate() {
      if(this.fromFun === 'system'){
        this.$fileDownload('/user/export/template');
      }else if(this.fromFun === 'workSpace'){
        this.$fileDownload('/user/export/template/' + this.fromFun);
      }
    },
    upload(file) {
      this.isLoading = false;
      ...
      let url = '/user/import';
      if(this.fromFun === 'workSpace'){
        url = '/user/import/'+ this.fromFun + "/" + getCurrentWorkspaceId();
      }

      this.result = this.$fileUpload(url, file.file, null, {}, response => {
