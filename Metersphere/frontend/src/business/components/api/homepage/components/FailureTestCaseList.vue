    ...
    <el-table border :data="tableData" class="adjust-table table-content" height="300px">
      ...
      <el-table-column prop="caseName"  :label="$t('api_test.home_page.failed_case_list.table_coloum.case_name')" width="150">
        <template v-slot:default="{row}">
            <el-link type="info" @click="redirect('caseGoTo',row.id)">
              {{ row.caseName }}
            </el-link>
        </template>
      </el-table-column>
      <el-table-column
        ...
        <template v-slot:default="scope">
          ...
          <ms-tag v-if="scope.row.caseType == 'functional'" type="danger" effect="plain" :content="$t('api_test.home_page.failed_case_list.table_value.case_type.functional')"/>
        </template>
      </el-table-column>
    ...
    redirect(pageType,param){
      switch (pageType){
        case "testPlanEdit":
          ...
          break;
        case "caseGoTo":
          this.$emit('redirectPage',pageType, null, param);
          break;
        case "apiCase":
