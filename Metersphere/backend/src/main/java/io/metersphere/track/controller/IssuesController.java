...
//    }
    ...
    @PostMapping("/list/relate/{goPage}/{pageSize}")
    public Pager<List<IssuesDao>> relateList(@PathVariable int goPage, @PathVariable int pageSize, @RequestBody IssuesRequest request) {
        if(IssuesManagePlatform.Jira.toString().equals(request.getPlatform())){
            Integer issuesTotal = issuesService.issueCount(request);
            Page<List<Issues>> page = new Page<>();
            page.setTotal(issuesTotal);
            page.setPageSize(pageSize);
            page.setPages(goPage);
            page.setCount(true);
            return PageUtils.setPageInfo(page, issuesService.syncJiraIssues(request,goPage,pageSize,""));
        }else{
            Page<List<Issues>> page = PageHelper.startPage(goPage, pageSize, true);
            return PageUtils.setPageInfo(page, issuesService.relateLocalIssues(request));
        }
    }
...
//    }
    ...
    @GetMapping("/get/{caseId}/{planId}")
    public List<IssuesDao> getIssuesList(@PathVariable String caseId, @PathVariable String planId) {
        return issuesService.getIssuesList(caseId, planId);
    }
